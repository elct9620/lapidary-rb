...
module Maintenance
  module Repositories
    # Repository for node rename operations.
    # Uses FK-safe ordering: create target first, migrate edges, then remove old node.
    class NodeRenameRepository
      ...
      def insert_cloned_node(old_id, new_id, data)
        old_node = nodes.where(id: old_id).first
        nodes.insert(id: new_id, type: old_node[:type], data: generate_json(data),
                     created_at: old_node[:created_at], updated_at: Time.now)
      end

      def migrate_edges(old_id, new_id)
        edges.where(Sequel.or(source: old_id, target: old_id)).all.each do |edge|
          old_key = Lapidary::EdgeKey.from_edge_row(edge)
      ...
      def migrate_single_edge(old_key, new_key)
        if edges.where(new_key.to_where).any?
          merge_edge_observations(old_key, new_key)
          edges.where(old_key.to_where).delete
        else
      ...
      def copy_edge(old_key, new_key)
        old_edge = edges.where(old_key.to_where).first
        edges.insert(
      ...
      def obs_by_edge(key)
        observations.where(key.to_observation_where)
      end
      ...
      def merge_node_data(target_id, source_data)
        target_node = nodes.where(id: target_id).first
        ...
        nodes.where(id: target_id).update(data: generate_json(merged), updated_at: Time.now)
      end

      def infer_display_name(old_id)
        result = Lapidary::ParentheticalName.parse(old_id.split('://', 2).last)
