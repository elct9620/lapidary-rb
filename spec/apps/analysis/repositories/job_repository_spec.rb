...
RSpec.describe Analysis::Repositories::JobRepository do
  ...
  let(:job) { Analysis::Entities::Job.new(entity_type: 'issue', entity_id: 1) }
  ...
  describe '#enqueue' do
    it 'inserts a pending job into the database' do
      ...
      expect(row[:entity_type]).to eq('issue')
      expect(row[:entity_id]).to eq('1')
  ...
  describe '#claim_next' do
    context 'when there are pending jobs' do
      ...
      it 'returns the job with correct attributes' do
        claimed = repository.claim_next
        expect(claimed.entity_type).to eq('issue')
        expect(claimed.entity_id).to eq('1')
      end
    ...
    context 'when jobs are scheduled in the future' do
      let(:future_job) do
        Analysis::Entities::Job.new(entity_type: 'issue', entity_id: 1, scheduled_at: Time.now + 3600)
      end
    ...
    context 'with multiple pending jobs' do
      before do
        repository.enqueue(Analysis::Entities::Job.new(entity_type: 'issue', entity_id: 1))
        repository.enqueue(Analysis::Entities::Job.new(entity_type: 'issue', entity_id: 2))
      end
